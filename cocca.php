...
function cocca_MetaData()
{
    return array(
        'DisplayName' => 'CoCCA EPP Registry',
        'APIVersion' => '1.0.1',
    );
}

function _cocca_error_handler($errno, $errstr, $errfile, $errline)
{
    if (!preg_match("/epp/i", $errfile)) {
        return true;
    }

    _cocca_log("Error $errno:", "$errstr on line $errline in file $errfile");
}

set_error_handler('_cocca_error_handler');
_cocca_log('================= ' . date("Y-m-d H:i:s") . ' =================');

function cocca_getConfigArray($params = array())
{
    _cocca_log(__FUNCTION__, $params);
    _cocca_create_table();
    _cocca_create_column();
...
function _cocca_startEppClient($params = array())
{
    $s = new cocca_epp_client($params);
    $s->login($params['clid'], $params['pw'], $params['registrarprefix']);
    return $s;
}

function cocca_RegisterDomain($params = array())
{
    _cocca_log(__FUNCTION__, $params);
    $return = array();
    try {
        $s = _cocca_startEppClient($params);
        $from = $to = array();
        $from[] = '/{{ name }}/';
...
function cocca_RenewDomain($params = array())
{
    _cocca_log(__FUNCTION__, $params);
    $return = array();
    try {
...
function cocca_TransferDomain($params = array())
{
    _cocca_log(__FUNCTION__, $params);
    $return = array();
    try {
...
function cocca_GetNameservers($params = array())
{
    _cocca_log(__FUNCTION__, $params);
    $return = array();
    try {
...
<epp xmlns="urn:ietf:params:xml:ns:epp-1.0"
  ...
  <command>
    ...
    <clTRID>{{ clTRID }}</clTRID>
        ...
        $status = array();
        Capsule::table('cocca_domain_status')->where('domain_id', '=', $params['domainid'])->delete();
        foreach($r->status as $e) {
            ...
            Capsule::table('cocca_domain_status')->insert(['domain_id' => $params['domainid'], 'status' => $st]);
        }
    }
...
function cocca_SaveNameservers($params = array())
{
    _cocca_log(__FUNCTION__, $params);
    $return = array();
    try {
...
function cocca_GetRegistrarLock($params = array())
{
    _cocca_log(__FUNCTION__, $params);
    $return = 'unlocked';
    try {
...
function cocca_SaveRegistrarLock($params = array())
{
    _cocca_log(__FUNCTION__, $params);
    $return = array();
    try {
...
function cocca_GetContactDetails($params = array())
{
    _cocca_log(__FUNCTION__, $params);
    $return = array();
    try {
...
function cocca_SaveContactDetails($params = array())
{
    _cocca_log(__FUNCTION__, $params);
    $return = array();
    try {
...
function cocca_IDProtectToggle($params = array())
{
    _cocca_log(__FUNCTION__, $params);
    $return = array();
    try {
...
function cocca_GetEPPCode($params = array())
{
    _cocca_log(__FUNCTION__, $params);
    $return = array();
...
function cocca_RegisterNameserver($params = array())
{
    _cocca_log(__FUNCTION__, $params);
    $return = array();
    try {
...
function cocca_ModifyNameserver($params = array())
{
    _cocca_log(__FUNCTION__, $params);
    $return = array();
    try {
...
function cocca_DeleteNameserver($params = array())
{
    _cocca_log(__FUNCTION__, $params);
    $return = array();
    try {
...
function cocca_RequestDelete($params = array())
{
    _cocca_log(__FUNCTION__, $params);
    $return = array();
    try {
...
function cocca_manageDNSSECDSRecords($params = array())
{
    _cocca_log(__FUNCTION__, $params);
    $return = array();
    try {
...
function cocca_ClientAreaCustomButtonArray()
{
    $buttonarray = array(
...
function cocca_AdminCustomButtonArray($params = array())
{
    _cocca_log(__FUNCTION__, $params);
    ...
    $domain = Capsule::table('cocca_domain_status')->where('domain_id', '=', $domainid)->where('status', '=', 'clientHold')->first();
...
function cocca_OnHoldDomain($params = array())
{
    _cocca_log(__FUNCTION__, $params);
    $return = array();
    try {
...
function cocca_UnHoldDomain($params = array())
{
    _cocca_log(__FUNCTION__, $params);
    $return = array();
    try {
...
function cocca_TransferSync($params = array())
{
    _cocca_log(__FUNCTION__, $params);
    $return = array();
    try {
...
function cocca_Sync($params = array())
{
    _cocca_log(__FUNCTION__, $params);
    $return = array();
    try {
...
function _cocca_modulelog($send, $responsedata, $action)
{
    $from = $to = array();
    $from[] = "/<clID>[^<]*<\/clID>/i";
    ...
    logModuleCall('cocca',$action,$sendforlog,$responsedata);
}

function _cocca_log($func, $params = false)
{

    // comment line below to see logs
    //return true;

    $handle = fopen(dirname(__FILE__) . '/cocca.log', 'a');
    ob_start();
    echo "\n================= $func =================\n";
    print_r($params);
...
function _cocca_create_table()
{
    ...
    if (!Capsule::schema()->hasTable('cocca_domain_status')) {
        try {
            Capsule::schema()->create('cocca_domain_status',
            ...
            echo "Unable to create table 'cocca_domain_status': {$e->getMessage() }";
        }
    }
}

function _cocca_create_column()
{
